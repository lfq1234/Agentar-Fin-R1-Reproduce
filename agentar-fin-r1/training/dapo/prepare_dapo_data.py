#!/usr/bin/env python3
"""Agentar-Fin-R1 — DAPO 数据预处理：原始对话 JSON → verl parquet。

将原始数据（messages 格式，role: HUMAN/ASSISTANT）转换为 DAPO 格式：
  - messages：仅保留 user 消息（prompt）
  - extra_info：原题 + 标准思维链 + 标准答案输出（供 RLAIF 裁判对标）

用法：
  python training/dapo/prepare_dapo_data.py \
      --input ./data/raw/train.json --output ./data/verl/dapo.parquet
"""
...
def main():
    ...
    print(f"[prepare_dapo] 加载: {args.input}")
    messages_list = load_messages(args.input)
    print(f"[prepare_dapo] 共 {len(messages_list)} 条")
    ...
    print(f"[prepare_dapo] 完成 → {args.output} ({len(df)} 条)")
