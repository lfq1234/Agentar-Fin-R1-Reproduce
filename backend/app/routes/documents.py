...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | ``/api/v1/documents`` | 上传（multipart：files[]） |
...
作用域：所有端点强制由 ``CurrentUser``（JWT 解析）提供 ``user_id`` 过滤（09 数据隔离），
不再接受请求体/查询参数的 ``user_id``（评审：user_id 信任源由 token 决定）。
...
async def list_documents(current_user: User = CurrentUser) -> DocumentListOut:
    rows = await run_in_threadpool(svc.list_documents, current_user.id)
    return DocumentListOut(documents=[_to_doc_out(r) for r in rows])


@router.get("/documents/{doc_id}/status", response_model=PersonalDocumentOut)
async def get_document_status(
    doc_id: str, current_user: User = CurrentUser
) -> PersonalDocumentOut:
    row = await run_in_threadpool(svc.get_status, doc_id, current_user.id)
    if row is None:
...
async def delete_document(doc_id: str, current_user: User = CurrentUser) -> Response:
    ok = await run_in_threadpool(svc.delete_document, doc_id, current_user.id)
    if not ok:
        raise HTTPException(status_code=404, detail="文档不存在")
...
async def get_knowledge_graph(current_user: User = CurrentUser) -> KnowledgeGraphOut:
    nodes, edges = await run_in_threadpool(svc.get_graph, current_user.id)
...
async def rag_retrieve(
    req: RagRetrieveRequest, current_user: User = CurrentUser
) -> RagRetrieveResponse:
